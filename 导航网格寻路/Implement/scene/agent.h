...
namespace NavMeshScene {
    ...
    class Scene;
/*
Every agent has unique filter and shared mapdata.
*/
    ...
    protected:
        //Move， the result is stored in realEndPos
        ...
        Agent* checkPosByAOI(float& dstX, float& dstY);

        uint64_t mId;
        float mHalfExtents[3];  //agent's extents
        float mPosition[3];     //current position
        float mVelocity[3];     //agent's velocity
        uint64_t mCurPolyRef;   //current polygon
